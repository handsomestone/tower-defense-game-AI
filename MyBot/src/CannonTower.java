...
public class CannonTower
{
	...
	public List<Target> Attack()
	{
		...
		int hurtLife =0;
		//System.out.println("cannonTower Task "+position.getRow()+" "+position.getCol());
		while(attackList.isEmpty() == false)
		{
			Target task = attackList.poll();
		/*	System.out.println("enemyID "+task.getID());
			System.out.println("enemyArriveTime "+task.arriveTime);
			System.out.println("enemyLeaveTime "+task.leaveTime);
			...
			else
			{
				...
				if((lastAttackTime+rechargeTime)<task.arriveTime)
				{
					hurtLife = attackPower;
					lastAttackTime = task.arriveTime;;
					while(lastAttackTime+rechargeTime<task.leaveTime)
		...
		attackList.clear();
		/*
		System.out.println("cannonTower attackList "+position.getRow()+" "+position.getCol());
		for(int h=0;h<hasBeenAttack.size();h++)
		{
			System.out.println("hID "+hasBeenAttack.get(h).ID);
			System.out.println("hhurtLIfe "+hasBeenAttack.get(h).getHurtLife());
			System.out.println("timeTocome "+hasBeenAttack.get(h).timeToCome);
			System.out.println("time To arrvie"+hasBeenAttack.get(h).arriveTime);
			System.out.println("time To leave"+hasBeenAttack.get(h).leaveTime);
		}*/
